...
class WeatherPipelineOrchestrator:
    ...
    def fetch_and_save_bronze(self, cities: List[str]) -> Optional[str]:
        ...
        logger.info(f"Fetched {len(api_responses)} raw API responses")

        # Save raw data as JSON locally
        timestamp = datetime.utcnow().strftime('%Y%m%d%H%M%S')
        local_path = f"/tmp/weather_raw_{timestamp}.json"

        # LƯU Ý SỬA LỖI Ở ĐÂY: Lưu trực tiếp api_responses (dữ liệu thô) xuống file JSON
        # Không dùng transform_to_entity ở bước này để giữ tính toàn vẹn của Bronze Layer
        try:
            with open(local_path, 'w') as f:
        ...
        logger.info(f"Bronze layer completed: {len(api_responses)} records saved")
        return object_name
    ...
    def transform_to_silver(self, bronze_path: str) -> Optional[str]:
        ...
        # Guard clause để tránh lỗi KeyError nếu dataframe rỗng
        if df.empty:
            logger.warning("No valid data available to transform after cleaning.")
            return None
